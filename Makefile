...
build: deps
	...
	$(GO) build -v -o bin/go-generated-views -ldflags='-X "main.version=example" -X "main.commit=example" -X "main.date=example" -X "main.builtBy=example"'  .
...
build_docker:
	KO_DOCKER_REPO=nickspring/go-generated-views VERSION=$(GITHUB_REF) COMMIT=$(GITHUB_SHA) DATE=$(DATE) BUILT_BY=$(USER) ko build --bare --local
